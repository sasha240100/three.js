...
var Player = function ( editor ) {
	...
	signals.startPlayer.add( function () {
		...
		function getClipFromObject(object, clipName) {
			return (
				object
				&& object.animations
				...
				scene
				&& scene.animations
				&& scene.animations.find(clip => clip.name === clipName)
			);
		}

		function playAnimationByData(object, data, playTriggerer = false) {
			const mixer = object.mixer;
			const name = data[1].alias || data[0];

			let clipObject = getClipFromObject(object, name);
    ...
    // @WORK AUTO
		scene.traverse(object => {
			...
			if (
				...
				const data = object.userData.__editor.button;
						...
						if (data.action) {
							switch (data.action) {
								case 'url':
									...
									break;
								case 'clip_select':
									const clipSelect = data.clipSelect;
									if (!clipSelect) return;

									scene.traverse((animationObject) => {
										if (
											animationObject.userData
											&& animationObject.userData.__editor
											&& animationObject.userData.__editor.animations
										) {
											if (animationObject.userData.__editor.animations[clipSelect]) {
												playAnimationByData(
													animationObject,
													[clipSelect, animationObject.userData.__editor.animations[clipSelect]]
												);
											}
										}
									})
			...
			if (
				...
				Object.entries(object.userData.__editor.animations).forEach((data) => playAnimationByData(object, data));
